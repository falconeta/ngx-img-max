...
export interface ImageSizeData {
  ...
  file: File;
  ignoreAlpha: boolean;
  compressedFileSubject: Subject<any>;
  maxSizeInMB: number;
...
export class ImgMaxSizeService {
  ...
  public compressImage(file: File, maxSizeInMB: number, ignoreAlpha: boolean = false, logExecutionTime: boolean = false): Observable<any> {
    ...
    const imageSizeData: ImageSizeData = {
      ...
      file,
      ignoreAlpha,
      compressedFileSubject,
      maxSizeInMB,
      logExecutionTime
    };
    imageSizeData.ctx = imageSizeData.cvs.getContext('2d');

    // let self = this;
    img.onload = this.imageLoaded.bind(this, imageSizeData);
    img.src = window.URL.createObjectURL(file);
    return compressedFileSubject.asObservable();
  }

  private imageLoaded(imageSizeData: ImageSizeData) {
  ...
  private generateResultFile(blob: Blob, name: string, type: string, lastModified: number): File {
    const resultFile = new Blob([blob], { type });
  ...
  private checkCompressionStatus(
    ...
    return new Promise((resolve, reject) => {
      if (quality === 100 && newQuality >= 100) {
        ...
        // COMPRESSION END
        // next steps quality would be the same quality but newQuality is slightly
        // bigger than old one, means we most likely found the nearest quality to compress to maximal size
        resolve(this.getResultFile(blob));
      } else if (currentStep > 5 && newQuality > quality && newQuality < quality + 2) {
        // COMPRESSION END
        // for some rare occasions the algorithm might be stuck around e.g. 98.5 and 97.4 because of
        // the maxQuality of 100, the current quality is the nearest possible quality in that case
        resolve(this.getResultFile(blob));
